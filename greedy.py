...
class TD3Agent:
    ...
    def build_critic_network(self, state_dim, action_dim):
        ...
        layer1 = tf.keras.layers.Dense(64, activation='relu')(concat_layer)
        layer2 = tf.keras.layers.Dense(64, activation='relu')(layer1)
    ...
    def update_critic(self, critic, target_critic, states, actions, rewards, next_states, dones):
        with tf.GradientTape() as tape:
            ...
            target_noise = tf.clip_by_value(tf.random.normal(target_actions.shape, stddev=0.2), -0.5, 0.5)
...
class UAVEnvironment:
    def __init__(self, aoi_iotds, scheduled_iotds, current_x, current_y, current_time_block, buffer_size):
        ...
        self.data_to_transmit = np.array([10 for _ in range(self.m)])
        self.c = 3e8
        ...
        self.observation_space = spaces.Box(low=np.array([0, 0] + [0] * self.m * 2),
                                            high=np.array([self.x_max, self.y_max] + [100] * self.m + [1]*self.m),
                                            dtype=np.float32)

        self.action_space = spaces.Box(low=np.array([0, 0, -1]),
                                        high=np.array([self.compute_max_distance(), self.v, 1]),
                                        dtype=np.float32)

    def compute_max_distance(self):
        return min(self.x_max - self.current_x, self.y_max - self.current_y)
    ...
    def step(self, action):
        ...
        for i in range(self.m):
            if self.current_x == self.iotd_loc[i][0] and self.current_y == self.iotd_loc[i][1]:
...
current_x = np.random.uniform(0, X_MAX)
current_y = np.random.uniform(0, Y_MAX)
